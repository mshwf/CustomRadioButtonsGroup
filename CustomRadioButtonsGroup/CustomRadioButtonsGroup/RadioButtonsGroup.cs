    ...
    public class RadioButtonsGroup : ContentView
    {
        ...
        private void RebuildOnItemsSource()
        {
            ...
            foreach (var item in items)
            {
                ...
                Label radioCircle = new Label { ClassId = "r", Text = unchckd, TextColor = FrontColor, FontSize = Device.GetNamedSize(NamedSize.Large, typeof(Label)), VerticalOptions = LayoutOptions.Center, VerticalTextAlignment = TextAlignment.Center };
        ...
        private void RadioChecked(object sender, EventArgs e)
        {
            ...
            var lb = stRadio.Children.First(x => x.ClassId == "r") as Label;
            if (lb.Text == unchckd)
            {
                if (SelectedIndex >= 0)
